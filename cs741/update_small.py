...
pre_computed_table = [[[None for _ in range(16)] for _ in range(16)] for _ in range(16)]
# print(len(pre_computed_table), len(pre_computed_table[0]), len(pre_computed_table[0][0]))
...
def is_mixture(crypt, k, x, y, z, w, ind):
	...
	crypt.key = key_guess
	c_x = map(ord, crypt.cipher(''.join(x)))
	c_y = map(ord, crypt.cipher(''.join(y)))
	c_z = map(ord, crypt.cipher(''.join(z)))
	c_w = map(ord, crypt.cipher(''.join(w)))

	return c_x[ind]^c_y[ind] == c_z[ind]^c_w[ind]
				...
				for k5 in keys_5:
					for k10 in keys_10:
						for k15 in keys_15:
							...
							if (c_x[0]^c_y[0] == c_z[0]^c_w[0]) and (c_x[4]^c_y[4] == c_z[4]^c_w[4]) and (c_x[8]^c_y[8] == c_z[8]^c_w[8]) and (c_x[12]^c_y[12] == c_z[12]^c_w[12]):
								return key_guess


def update_34(all_good_pairs):
	one_round_crypt = AES_128(1, True)
	one_round_crypt.key = ''.join([chr(0)]*16)
	pre_compute(one_round_crypt)
	len_gp = len(all_good_pairs) # 2**15
	...
	for i in range(len_gp):
		for j in range(i+1, len_gp):
			x_, y_ = all_good_pairs[i]
			z_, w_ = all_good_pairs[j]

			x, y = map(ord, x_), map(ord, y_)
			z, w = map(ord, z_), map(ord, w_)
...
key = "040102030404060708090a0b030d0e0f".decode('hex')

all_good_pairs = get_goodpairs(key)
# for g in all_good_pairs:
# 	print(g)
print(list(key))
